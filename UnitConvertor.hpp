...
namespace UnitConvertor
{
    ...
    static double toProperValue(double value)
    {
    ...
    ///模板本身会无限地归导致std::ratio分子/分母溢出,所以这里需要特化一个模板专门处理溢出的情况
    template<> double toProperValue<std::atto>(double value)
    { return value; }

    template<> double toProperValue<std::exa>(double value)
    { return value; }
