...
struct Trans
{
    ...
    byProtocol(Args...args)
    {
        ...
        std::vector<unsigned> protocolArray;
        generateOnebyteProtocolArray<BytePerArg...>(protocolArray);
    ...
    byProtocol(Args...args)
    {
        ...
        std::vector<unsigned> protocolArray;
        generateVarbyteProtocolArray<BytePerArg...>(protocolArray);
...
private:
    ...
    ///辅助类,用于减少数据转换的模板函数参数数量,将一些固定参数作为成员变量,避免每一次访问这些数据都要将其作为参数传递
    class Transor
    {
    public:
        Transor(ByteMode Mode,std::vector<unsigned>& Protocol,unsigned char* Data):m_Mode(Mode),m_Protocol(Protocol),m_Data(Data){}

        template<typename First,typename...Args>
        void transImpl(unsigned index,First first,Args...args)
        {
            unsigned Byte = m_Protocol.at(index);
            for(unsigned i = 0; i < Byte; i++)
            {
                unsigned offset = (m_Mode == Big) ? (Byte - i - 1) * CharBit : (i *CharBit );
                m_Data[index+i] = static_cast<unsigned char>(first >> offset) & 0xFF ;
            }

            transImpl(index+Byte,std::forward<Args>(args)...);
        }

        template<typename First>
        void transImpl(unsigned index,First first)
        {
    ...
    private:
        ...
        std::vector<unsigned> m_Protocol;
    };

    ///生成数据占用若干字节的通信协议数组
    template<unsigned First,unsigned...Remains>
    static void generateVarbyteProtocolArray(std::vector<unsigned>& array)
    {
        array.push_back(First);
        generateVarbyteProtocolArray(array,Remains...);
    }
    ...
    ///生成数据只占用一字节的通信协议数组
    template<unsigned Bytes>
    static void generateOnebyteProtocolArray(std::vector<unsigned>& array)
    {
        array.resize(Bytes);
